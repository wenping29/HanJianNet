...
function normalizeMenuItem(raw: Record<string, unknown>): MenuItem {
  const childrenRaw = (raw.children ?? raw.Children ?? []) as Record<string, unknown>[]
  return {
    ...
    children: childrenRaw.length > 0 ? childrenRaw.map(normalizeMenuItem) : undefined,
  }
}
...
export const api = {
  ...
  menus: async () => {
    const res = (await request('/admin/menus')) as { items: Record<string, unknown>[] }
    return { items: res.items.map(normalizeMenuItem) }
  },
  ...
  adminRevisionStats: () =>
    request<{ stats: RevisionStatusStats }>('/admin/revisions/stats/status'),
