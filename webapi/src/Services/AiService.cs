...
public class AiService(IOptions<DeepSeekOptions> options, IHttpClientFactory httpFactory, ILogger<AiService> logger)
{
    ...
    public async Task<AiTraitorResultDto> QueryTraitorAsync(string name, CancellationToken ct = default)
    {
        ...
        var content = await ChatAsync(SystemPrompt, $"请检索并整理以下历史人物的生平史料：\n姓名：{name}", ct);
        return Sanitize(DeserializeAiResult(ExtractJson(content)), name);
    }

    public async Task<AiEventResultDto> QueryEventAsync(string name, CancellationToken ct = default)
    {
        name = name?.Trim() ?? "";
        if (name.Length == 0)
            throw new ArgumentException("事件名称不能为空。");
        if (name.Length > 60)
            throw new ArgumentException("事件名称过长，无法查询。");

        var content = await ChatAsync(EventSystemPrompt, $"请检索并整理以下历史事件（惨案/宏观事件）的史料：\n事件：{name}", ct);
        return SanitizeEvent(DeserializeEventResult(ExtractJson(content)), name);
    }

    /// <summary>统一的 DeepSeek 对话调用，返回 AI 输出的纯文本内容。</summary>
    private async Task<string> ChatAsync(string systemPrompt, string userPrompt, CancellationToken ct)
    {
    ...
    private static AiEventResultDto DeserializeEventResult(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<AiEventResultDto>(json, JsonOpts.Default)
                ?? throw new InvalidOperationException("AI 返回内容无法解析，请重试。");
        }
        catch (JsonException)
        {
    ...
    private static AiEventResultDto SanitizeEvent(AiEventResultDto result, string fallbackName)
    {
        result.Name = string.IsNullOrWhiteSpace(result.Name) ? fallbackName : result.Name.Trim();
        result.Alias = NullIfEmpty(result.Alias);
        result.EventType = NullIfEmpty(result.EventType);
        result.Era = NormalizeEra(result.Era);
        result.Year = ValidYear(result.Year);
        result.Province = NullIfEmpty(result.Province);
        result.City = NullIfEmpty(result.City);
        result.Location = NullIfEmpty(result.Location);
        result.Summary = NullIfEmpty(result.Summary) ?? "";
        result.Keywords = CleanList(result.Keywords);
        if (result.PersonCount is < 0 or > 1_000_000) result.PersonCount = null;
        return result;
    }
    ...
    private const string EventSystemPrompt =
        "你是中国近代史史料整理助手，专研民国与抗日战争时期的历史事件（包括各类惨案、屠杀、侵略暴行与宏观抗战事件）。请根据给定事件名称检索并整理其史料，只输出一个 JSON 对象，不要输出 JSON 以外的任何文字、不要用 Markdown。" +
        "JSON 结构如下：" +
        "{ \"name\":\"事件主名\", \"alias\":\"别名\", \"eventType\":\"事件类型\", \"era\":\"民国\", \"year\":1937, \"province\":\"省\", \"city\":\"市/县\", \"location\":\"具体地点\", \"isGeneral\":false, \"personCount\":3500, \"summary\":\"200-400字事件概述，叙述时间地点经过与伤亡影响\", \"keywords\":[\"关键词1\",\"关键词2\"] }" +
        "要求：1. 基于真实可考史料，宁缺毋滥，宁可留空也不要编造；2. year 用公元年、无法确证填 null；3. era 仅可取值：宋末、明末、清末、民国、抗日战争时期、其他；4. isGeneral 表示是否宏观事件（宏观为 true，具体惨案为 false），personCount 填已知的伤亡或受影响人数，无确切数字填 null；5. summary 叙述事件发生时间、地点、经过、伤亡与历史影响，字符串为纯文本、不用 Markdown；6. 若该名称无可靠史料或所指不明确，仍按结构输出、不确定字段留空；7. keywords 提炼 3-8 个检索/归档用关键词。";
}